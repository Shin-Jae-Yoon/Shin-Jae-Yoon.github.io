...
웹 애플리케이션을 Model, View, Controller 세 역할로 나누는 개발 방법론. 목적은 [[응집도와 결합도|모듈 간 결합도를 낮추고 응집도를 높이는 것]]이다.
...
Model이 곧 도메인인데, 도메인은 화면과 UI와 기술 인프라를 뺀 나머지, 시스템이 구현해야 할 핵심 업무 영역이다. 그래서 컨트롤러는 도메인이 아니다. 엔티티와 리포지토리가 도메인이다. 이 구분이 나중에 [[레이어드 아키텍처]]와 [[DDD]]로 이어진다.
...
서비스가 커지면 컨트롤러와 모델만으로는 부족해진다. 요청을 받는 일과 비즈니스 규칙을 수행하는 일과 데이터를 꺼내는 일이 뒤섞이기 시작한다. Model 쪽을 다시 나눈 것이 [[레이어드 아키텍처]]다.
