...
[[다형성]]이 이것을 가능하게 한다. 인터페이스를 두면 새 구현을 추가하는 것만으로 동작이 바뀐다.

## 리스코프 치환 원칙 (LSP)

추상체가 쓰이는 자리에 구상체가 들어가도 아무 문제가 없어야 한다. 추상과 구체의 관계가 분명하면 저절로 지켜진다. 깨지는 것은 공통 코드를 빼려고 상속을 썼을 때다. 부모가 진짜 추상체가 아니니 자식이 부모 자리를 대신할 수 없다. [[상속과 합성]]에 자세히 있다.
...
상위 모듈은 하위 모듈에 의존하지 않는다. 둘 다 추상화에 의존해야 한다. 다섯 중 가장 자주 언급되고 파급도 크다. [[DIP]]에 따로 정리해두었다.
