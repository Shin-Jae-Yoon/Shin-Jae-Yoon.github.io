...
자바스크립트가 값을 여럿 담는 두 그릇과 그것을 다루는 함수들. 자바의 [[컬렉션 프레임워크|List와 Map]], 파이썬의 [[파이썬 자료형|리스트와 딕셔너리]]와 같은 구도다.
...
반복문을 대신하는 고차 함수도 배열 쪽에 있다. 자바 [[스트림]]의 연산과 이름까지 같다.
...
반환값이 음수면 a가 앞, 양수면 b가 앞이다. 자바의 [[Comparable과 Comparator|Comparator]]와 정확히 같은 약속이다.
