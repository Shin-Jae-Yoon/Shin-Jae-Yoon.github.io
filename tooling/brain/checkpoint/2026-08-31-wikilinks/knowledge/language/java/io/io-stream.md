...
Java IO는 조립해서 쓰도록 만들어졌다. [[데코레이터 패턴]]이 그 구조다. 어디서 읽고 쓸지를 정하는 쪽이 주인공, 그 위에 어떻게 읽고 쓸지를 얹는 쪽이 장식이다. 주인공에게는 1바이트나 1문자 단위로 읽고 쓰는 메서드만 있고, 장식은 그것을 불러다 쓰면서 더 편한 메서드를 붙인다.
...
버퍼링이 속도를 바꾸는 폭은 크다. 버퍼 없이 쓰면 호출할 때마다 실제로 출력 장치에 나가므로 백만 번 부르면 백만 번의 [[시스템 콜]]이다. 버퍼를 쓰면 메모리에 모아뒀다가 한꺼번에 내보내니 실제 나가는 횟수가 확 준다. 100만 줄 출력에서 30초와 1초의 차이가 여기서 난다([[입출력 성능]]).
