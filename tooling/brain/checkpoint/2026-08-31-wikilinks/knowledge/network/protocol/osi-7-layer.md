...
패킷이 오가는 과정도 택배와 나란히 놓으면 그대로 대응된다. 책을 박스에 포장하듯 데이터를 Packet으로 만들고, 현관으로 나가듯 인터페이스로 나가고, 택배 기사에게 건네듯 게이트웨이에 전달한다. 물류 체계가 모아 분류하는 자리에서 게이트웨이가 [[스위치와 라우터|라우팅]]을 하고, 목적지 IP까지 간 다음에는 받는 사람 이름을 보고 물건을 건네듯 포트 번호를 보고 프로세스에 넘긴다.
