...
직접 하면 대가가 따른다. 서비스 코드가 트랜잭션 코드로 지저분해지고, 커넥션을 어떻게 물고 있을지 직접 관리해야 하고, 데이터 액세스 기술에 코드가 묶여 JDBC에서 JPA로 바꾸면 전부 고쳐야 하고, 비즈니스 로직과 관심사가 다른 코드가 한자리에 섞인다. 이 넷을 걷어내는 것이 [[선언적 트랜잭션]]이다.
