...
[[서브쿼리]]의 결과를 `NOT IN`에 쓸 때 자주 걸린다. "2000년대 생이 없는 부서의 ID와 이름"을 뽑는데 아직 부서를 배치받지 않아 `dept_id`가 NULL인 사원이 하나라도 있으면, 바깥 `D.id`가 무슨 값이든 FALSE 아니면 UNKNOWN이라 결과가 통째로 빈다.
