...
[[삽입 정렬]], [[선택 정렬]], [[버블 정렬]], [[힙 정렬]], [[퀵 정렬]]이 제자리 정렬이고 [[합병 정렬]]만 아니다. 합병 정렬이 예외인 이유는 두 조각을 합칠 때 결과를 담을 임시 배열이 필요하기 때문이다. 그 크기가 원소 개수에 비례하니 O(n)이라 무시할 수 없다.

[[퀵 정렬]]은 교환만 하므로 배열 자체는 제자리인데, 재귀 호출 때문에 [[콜 스택]]을 O(log n)만큼 쓴다. 보통은 이 정도를 제자리로 친다.
...
외부 정렬의 쪼개서 처리하고 합친다는 발상은 [[합병 정렬]]의 합치는 단계와 같고, 데이터를 감당 가능한 덩어리로 잘라 처리한다는 점에서 [[스프링 배치]]의 청크와도 닿아 있다.
