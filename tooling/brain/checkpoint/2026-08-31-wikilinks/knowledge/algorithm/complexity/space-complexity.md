...
공간복잡도는 대개 시간복잡도와 맞바꾸는 관계다. 메모리를 더 써서 시간을 줄이는 것이 [[카운트 배열]]이나 [[동적 계획법]]이고, 반대로 [[제자리 정렬]]은 추가 메모리를 안 쓰는 대신 원본을 훼손한다.

둘 중 무엇이 급한지는 문제가 정해준다. 제한을 먼저 읽고 시작하는 습관이 중요한 이유다.

## 2차원 배열과 재귀 깊이

2차원 배열에서 가장 자주 막힌다. N이 1만이면 `int[10000][10000]`은 1억 개, 400MB다. N² 크기 배열은 N이 조금만 커져도 못 잡는다. [[플로이드-워셜]]이 정점 수가 적을 때만 쓰이는 이유이기도 하다.

값을 인덱스로 쓰는 [[카운트 배열]]도 같은 이유로 값의 범위가 작을 때만 성립한다.

재귀 호출이 깊어질 때는 배열이 아니라 [[콜 스택]]이 쌓여서 터진다. 이건 메모리 제한이 아니라 스택 오버플로로 나타난다.
