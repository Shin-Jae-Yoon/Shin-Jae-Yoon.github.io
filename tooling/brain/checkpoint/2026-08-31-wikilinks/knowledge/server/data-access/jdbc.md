...
부여하는 방법은 셋이다. JDBC를 직접 쓰거나, [[SQL Mapper]]를 쓰거나, [[ORM과 JPA|ORM]]을 쓴다. 뒤의 둘을 영속성 프레임워크라고 묶어 부르는데, 모든 영속성 프레임워크가 내부적으로 JDBC API를 쓴다. JDBC가 바닥에 있는 이유다.
...
이 셋을 대신 처리해주는 것이 영속성 프레임워크다. [[JdbcTemplate과 스프링 데이터 JPA|JdbcTemplate]]은 반복 코드를 걷어내고, 스프링은 DB별 예외를 일관된 예외 계층으로 바꿔준다.
