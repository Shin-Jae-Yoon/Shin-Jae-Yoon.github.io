...
예외 처리에서 갈리는 것도 위치 때문이다. 인터셉터는 스프링 컨텍스트 안이라 `@ControllerAdvice`와 `@ExceptionHandler`로 예외를 처리할 수 있고, 필터는 그 장치가 닿지 않아 `doFilter()` 주변을 `try-catch`로 직접 감싼다. 자세한 내용은 [[스프링 예외 처리]]에 있다.
...
[[AOP]]까지 셋을 나란히 놓으면 층이 다르다. 필터는 서블릿 층, 인터셉터는 스프링 MVC 층, AOP는 메서드 층이다. 인터셉터와 AOP를 가르는 실질적인 기준은 [[Spring AOP와 AspectJ|파라미터]]다.
