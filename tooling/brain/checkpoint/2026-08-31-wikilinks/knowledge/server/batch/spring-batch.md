...
청크 단위로 트랜잭션을 수행하므로 실패하면 그 청크만큼만 롤백된다. 100만 건을 처리하다 90만 번째에서 실패해도 앞의 것은 남는다. 긴 작업을 처음부터가 아니라 이어서 할 수 있는 이유가 이것이다. 감당 가능한 덩어리로 잘라 처리한다는 발상은 [[제자리 정렬|외부 정렬]]에도 있다.
