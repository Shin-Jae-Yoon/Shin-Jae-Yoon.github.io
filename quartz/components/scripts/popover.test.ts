...
describe("showPopover on cache-hit with hash", () => {
  test("restores the active anchor after clearing the previous popover", async () => {
    ...
    assert.match(
      source,
      /function showPopover[\s\S]*?clearActivePopover\(\)\s*activeAnchor = link\s*popoverElement\.classList\.add\("active-popover"\)/,
    )
  })
