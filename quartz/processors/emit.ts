...
import { finalizeSeoOutput } from "../util/seoOutput"
...
export async function emitContent(ctx: BuildCtx, content: ProcessedContent[]) {
  ...
  if (cfg.configuration.baseUrl) {
    const rewrittenAliases = await finalizeSeoOutput(argv.output, cfg.configuration.baseUrl)
    emittedFiles += 1
    if (ctx.argv.verbose) {
      console.log(`[seo] robots.txt; ${rewrittenAliases} absolute alias canonical(s)`)
    }
  }
