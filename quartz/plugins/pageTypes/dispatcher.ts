...
function virtualPageData(cfg: GlobalConfiguration, vp: VirtualPage) {
  const value = cfg.generatedPageDate
  if (!value) return vp.data
  const date = new Date(value)
  if (Number.isNaN(date.getTime())) throw new Error(`Invalid generatedPageDate: ${value}`)
  return {
    frontmatter: {
      title: vp.title,
      tags: [],
