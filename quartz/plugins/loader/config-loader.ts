...
export async function loadQuartzLayout(layoutOverrides?: {
  ...
  const mergedDefaults = mergeLayoutOverride(defaultLayout, layoutOverrides?.defaults)
  ...
  if (layoutOverrides?.byPageType) {
    for (const [pageType, overrideLayout] of Object.entries(layoutOverrides.byPageType)) {
      mergedByPageType[pageType] = mergeLayoutOverride(
        mergedByPageType[pageType] ?? {},
        overrideLayout,
      )
    }
  }
...
function mergeLayoutOverride(
  base: Partial<FullPageLayout>,
  override?: Partial<FullPageLayout>,
): Partial<FullPageLayout> {
  if (!override) return base

  const merged = { ...base, ...override }
  if (base.header || override.header) {
    merged.header = [...(override.header ?? []), ...(base.header ?? [])]
  }
  return merged
}
...
function applyConditionWrapper(component: QuartzComponent, conditionName: string): QuartzComponent {
  ...
  if (!predicate) {
    throw new Error(
      `Unknown layout condition "${conditionName}". Register it before loading Quartz config or layout.`,
    )
  }
