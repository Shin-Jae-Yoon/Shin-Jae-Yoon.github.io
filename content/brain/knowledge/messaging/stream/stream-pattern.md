...
[[stream-processing|스트림 처리]] 애플리케이션이 반복해서 취하는 모양들. 상태가 필요한지, 전체 정보가 필요한지에 따라 갈린다.
...
윈도우 집계처럼 상태가 필요한 경우다. 그룹별 집계는 공유 상태가 아닌 로컬 상태로 해야 하고, 애플리케이션이 로컬 상태를 보유하는 순간 메모리와 영속성과 리밸런싱을 전부 고려해야 한다. 그 셋을 어떻게 다루는지는 [[stream-state|상태를 가진 스트림 처리]]에 적었다.
