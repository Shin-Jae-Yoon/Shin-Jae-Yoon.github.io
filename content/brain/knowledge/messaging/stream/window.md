...
이동 평균, 이번 주 가장 많이 팔린 상품, 시스템의 99분위 부하가 전부 윈도우 작업이다. [[stream-table-duality|두 스트림을 조인하는 것]]도 윈도우 작업인데, 같은 시간 간격 안에 발생한 이벤트끼리 맞춰야 하기 때문이다.
...
갱신 가능 기한은 이벤트가 해당 윈도우에 추가될 수 있는 마감이다. 이벤트가 최대 4시간까지 지연될 수 있다면 그때까지는 결과를 다시 계산해 갱신하고, 그 이상 지연된 것은 무시한다. [[stream-time|비순차 이벤트]]를 얼마나 받아줄 것인가의 문제이고, 오래 열어둘수록 로컬 상태의 메모리를 더 쓴다.
