...
[[linked-list|연결 리스트]]는 k번째 자리를 찾는 데 $O(N)$이 걸린다. 반복자를 한 번 그 자리에 잡아두면 바로 옆의 삭제와 탐색은 $O(1)$이다. 이 구조의 성능을 실제로 뽑아 쓰려면 인덱스 대신 반복자로 다뤄야 한다.
...
하나씩 꺼내 쓰는 순간 묶음으로 다루던 이점은 잃는다. 컬렉션 전체에 `map`이나 `filter` 같은 함수를 건네는 방식이 불가능해지기 때문이다. 그것을 되찾으려고 나온 것이 [[stream|스트림]]이다.
