...
메서드에 붙이는 것은 그 타입이 그 메서드에서만 쓰일 때다. 클래스에는 `<T>`밖에 없는데 변환 결과 타입 `<U>`가 필요하면 메서드 쪽에 선언해야 한다. [[functional-interface|함수형 인터페이스]]를 받는 `map`이 그런 모양이다.

[[collection-framework|컬렉션 프레임워크]]는 `List<E>`나 `Map<K, V>`처럼 대부분 제네릭으로 되어 있고, 쓰지 않으면 `Object`로 저장된다. 컬렉션과 제네릭을 늘 같이 쓰게 되는 이유다.
