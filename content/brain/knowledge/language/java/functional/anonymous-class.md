...
[[interface|인터페이스]]와 추상 클래스는 인스턴스를 만들 수 없어서 구현 클래스를 따로 두어야 했다. 한 번 쓰고 말 구현에 클래스 파일을 하나 만드는 것이 번거로웠고, 익명 클래스가 그 수고를 없앤다. 재사용할 일이 없다고 판단한 자리에 쓴다.
...
추상 메서드를 하나만 가져서 이렇게 줄일 수 있는 인터페이스를 [[functional-interface|함수형 인터페이스]]라고 부른다.
