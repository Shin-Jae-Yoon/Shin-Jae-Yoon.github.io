...
향상된 `for`문은 [[iterator|Iterator]]를 써서 컬렉션을 처음부터 끝까지 훑는다. 인덱스를 다루지 않아도 되니 가장 짧다.
...
[[js-variable|truthy와 falsy]] 때문에 조건 판단이 자바와 다르게 동작한다. `0`, `""`, `null`, `undefined`, `NaN`이 거짓으로 취급된다.
