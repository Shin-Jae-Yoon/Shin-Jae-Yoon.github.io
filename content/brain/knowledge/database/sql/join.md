...
두 개 이상의 테이블에 있는 데이터를 한 번에 조회하는 것. 관계형 DB는 데이터를 여러 테이블에 나눠 담고 [[key|외래키]]로 이어놓으므로, 실제로 쓸 만한 조회는 대부분 조인이다.
...
조인 컬럼이 NULL인 행은 내부 조인 결과에 들어가지 못한다. [[null-and-three-valued-logic|NULL과의 비교는 UNKNOWN]]이라 조건을 만족하지 못하기 때문이다. 부서가 배정되지 않은 직원은 사라진다.
