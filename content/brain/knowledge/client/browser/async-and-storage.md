...
네트워크로는 문자열만 보낼 수 있으므로 변환이 필요하다. 자바의 [[serialization|직렬화]]와 같은 문제를 푸는 것이고, JSON은 언어에 매이지 않는다는 점이 다르다.
...
`setTimeout`도 `fetch`도 기다리지 않고 등록해두고 다음 줄로 넘어간다. 자바스크립트는 [[rendering|스레드가 하나]]라 기다리면 화면 전체가 멈추기 때문에, 결과가 오면 실행할 함수를 콜백으로 등록해두고 다음 일을 한다.

## 관련

- [[dom-and-event|DOM과 이벤트]]
