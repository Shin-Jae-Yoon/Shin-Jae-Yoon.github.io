...
스레드 수에는 답이 있다. Goetz가 권한 값은 CPU 개수에 하나를 더한 만큼이다. 듀얼 코어에 스레드를 4개 두면 [[context-switching|컨텍스트 스위칭]]이 자주 일어나 오버헤드만 커진다. 어차피 CPU가 쉬지 않고 도는데 전환만 늘어나는 셈이다. 코어 수만큼 두면 전환이 거의 없어 CPU를 낭비하지 않고, 하나를 더 두는 것은 어떤 스레드가 잠깐 멈췄을 때 코어가 놀지 않게 하려는 여유분이다.
