...
싱글톤 스코프의 빈은 상태를 가지면 안 된다. 빈이 `value`라는 상태를 갖고 있고 스레드 1이 그 값을 증가시키는 사이 스레드 2가 같은 값을 가져다 쓴다면, 빈의 상태를 예측할 수 없어 의도한 결과가 나온다고 보장할 수 없다. [[servlet|서블릿]]에 상태를 두면 안 되는 것과 같은 이야기다.
