...
객체 하나가 모든 것을 하지도 않는다. 메시지를 주고받으며 함께 일을 끝내기 때문에 좋은 객체의 조건에 협력이 들어간다. 협력하려면 상대를 알아야 하는데, 상대의 구체적인 모습까지 알면 결합이 강해진다. 그래서 [[abstraction|추상 타입]]으로 알고 [[dependency-injection|밖에서 받는다]].
