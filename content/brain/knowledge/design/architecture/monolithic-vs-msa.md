...
[[layered-architecture|레이어드 아키텍처]]로 잘 나눠도 트래픽이 몰리면 한계가 온다. 일부 기능의 수요가 급증했는데 전체를 함께 확장해야 하는 상황이다. 그때 나오는 두 갈래다.
...
확장하려면 전체를 확장해야 한다. 보통 서버 스펙을 올리는 수직 확장이다. 배포도 느려서 한 줄 고쳐도 전체를 다시 빌드하고 올린다. 배포가 느린 문제는 [[zero-downtime-deploy|무중단 배포]]로 던다.

확장 문제는 패턴 안에서도 완화할 수 있다. 멀티 모듈로 나눠두면 나중에 서비스가 커졌을 때 확장하기 쉽고, [[cohesion-and-coupling|응집도는 올라가고 결합도는 내려간다]].
...
여러 서비스에 걸친 작업은 하나의 [[acid|트랜잭션]]으로 묶을 수 없다. 그래서 묶기를 포기하고 다른 방법을 쓴다. 보상 트랜잭션은 실패했을 때 되돌리는 작업을 따로 수행해 일관성을 유지하고, 복합 서비스는 트랜잭션을 지원하는 네이티브 프로토콜로 두 시스템을 묶어 API로 노출한다. 유스케이스를 하나 두고 최종 결과가 맞는지 검증하게 하는 방법도 있다.
