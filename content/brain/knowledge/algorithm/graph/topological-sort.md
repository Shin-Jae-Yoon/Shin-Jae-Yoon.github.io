...
[[dfs|DFS]]로 내려가다가 더 갈 곳이 없으면 그때 스택에 넣는다. 다 끝나고 스택을 꺼내면 그것이 위상 정렬 결과다. 먼저 끝난 것이 뒤에 오기 때문이다. 복잡도는 역시 O(V + E)다.

이쪽은 시작점 하나로 끝나지 않는다. 아직 방문하지 않은 정점이 남았으면 거기서 DFS를 새로 시작해야 하는데, 그 이유는 [[brain/knowledge/algorithm/data-structure/graph|그래프]]에 적어두었다.
