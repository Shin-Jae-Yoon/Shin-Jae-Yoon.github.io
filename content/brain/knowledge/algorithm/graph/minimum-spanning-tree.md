...
정점이 N개면 간선은 정확히 N-1개다. 모두 연결되어 있으면서 사이클이 없는 것이 [[tree|트리]]의 정의이기 때문이다.
...
같은 그룹인지 판단하는 데 [[union-find|유니온 파인드]]가 쓰인다. 정렬에 O(E log E), 간선마다 union-find에 O(log N)이 들어 O(E log N)이니 합치면 정렬이 지배해서 O(E log E)다.
...
프림과 [[dijkstra|다익스트라]]는 거의 같은 알고리즘이다. 두 줄만 다르고, 그 두 줄은 `dist` 배열이 무엇을 뜻하느냐에서 온다.
