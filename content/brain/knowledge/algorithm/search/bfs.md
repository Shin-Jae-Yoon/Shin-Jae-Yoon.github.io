...
[[queue-and-deque|큐]]를 쓴다. 한 정점을 방문하면 그 이웃 중 아직 안 가본 곳을 전부 큐에 넣고, 큐 앞에서 하나씩 꺼내 같은 일을 반복한다. 큐가 먼저 들어온 것을 먼저 내주니 가까운 것부터 처리되는 순서가 저절로 지켜진다.
...
방문 표시는 큐에 넣을 때 해야 한다. 꺼낼 때 하면 같은 정점이 큐에 여러 번 들어간다. 복잡도는 O(V + E)로 [[dfs|DFS]]와 같다.
...
그래서 최소 이동 횟수, 미로 탈출, 연결 요소 찾기, 진입차수 방식의 [[topological-sort|위상 정렬]]에 쓴다.
...
가중치가 다르면 [[dijkstra|다익스트라]]나 [[floyd-warshall|플로이드-워셜]]을 쓴다.

## 관련

- [[dfs|DFS]]
- [[dijkstra|다익스트라]]
- [[queue-and-deque|큐와 덱]]
