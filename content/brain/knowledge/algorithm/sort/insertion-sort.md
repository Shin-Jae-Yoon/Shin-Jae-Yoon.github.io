...
공간복잡도는 O(1)이다. 다만 새 배열을 만들어 거기 넣는 식으로 구현하면 O(n)이 되니, 원본 안에서 밀어내는 방식이어야 [[in-place-sort|제자리 정렬]]이다. 비교 대상이 새 원소보다 클 때만 뒤로 밀기 때문에 같은 값을 만나면 그 뒤에 자리를 잡고, 그래서 [[stable-sort|안정 정렬]]이다.
